...
class StockDataRepository:
    ...
    async def get_available_stocks(
        ...
        logger.debug(
            f"Fetched and cached {len(stocks)} available stocks (page {page}/{total_pages})"
        )
    ...
    async def get_stock_data(
        ...
        count_query = f"""
            ...
            AND time <= $3
        """
        count_result = await self.query_executor.execute_query(
            count_query, symbol, start_date, end_date
        )
        total_count = count_result[0]["total"] if count_result else 0
        ...
        data = await self.query_executor.execute_query(
            query, symbol, start_date, end_date, page_size, offset
        )
        ...
        total_pages = (total_count + page_size - 1) // page_size
        logger.debug(
            f"Fetched and cached stock data: {symbol} {start_date} to {end_date} page {page}/{total_pages} ({len(data)} records)"
        )

        return data, total_count, date_range

    async def get_stock_data_batch(
        self,
        symbols: List[str],
        start_date: date,
        end_date: date,
        ...
        try:
            ...
            results = await self.query_executor.execute_query(
                query, *[s.upper() for s in symbols], start_date, end_date
            )
    ...
    async def validate_date_range_has_data(
        ...
        try:
            ...
            return {
                ...
                "sufficient_data": coverage_pct
                >= 50,  # At least 50% coverage for meaningful backtest
            }
        except Exception as e:
    ...
    async def validate_stock_tradeable(self, symbol: str, check_date: date) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error checking if {symbol} was tradeable on {check_date}: {e}"
            )
    ...
    async def get_eligible_stocks_for_period(
        ...
        query = (
            "SELECT symbol FROM get_eligible_stocks_for_period($1, $2) ORDER BY symbol"
        )
    ...
    async def validate_symbols_for_period(
        ...
        for symbol in symbols:
            ...
            start_tradeable = await self.validate_stock_tradeable(
                symbol_upper, start_date
            )
