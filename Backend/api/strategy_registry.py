...
class StrategyMetadata:
    ...
    parameters: List[StrategyParameter] = field(default_factory=list)  # Configuration parameters
    category: str = "technical"                                # Strategy category (technical, fundamental, etc.)
    risk_level: str = "medium"                                 # Risk assessment: low, medium, high
    requires_indicators: List[str] = field(default_factory=list)  # Required technical indicators
...
class StrategyRegistry:
    ...
    def __init__(self):
        ...
        self._strategies: Dict[str, Type[StrategyInterface]] = {}     # Registered strategy classes
        self._metadata_cache: Dict[str, StrategyMetadata] = {}        # Cached strategy metadata
        ...
        logger.info(f"Strategy registry initialized with {len(self._strategy_paths)} discovery paths")
    ...
    def register_strategy(self, strategy_class: Type[StrategyInterface]) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Failed to instantiate strategy {strategy_class.__name__}: {e}")
                return False
            ...
            if not metadata.strategy_id.replace('_', '').isalnum():
                logger.error(f"Strategy {strategy_class.__name__} has invalid strategy_id format: '{metadata.strategy_id}'")
                return False
            ...
            if metadata.strategy_id in self._strategies:
                ...
                if existing_metadata:
                    logger.warning(f"Strategy {metadata.strategy_id} already registered "
            ...
            logger.info(f"Successfully registered strategy: {metadata.strategy_id} "
                       f"({metadata.display_name}) v{metadata.version} by {metadata.author}")
            return True
            
        except Exception as e:
            logger.error(f"Unexpected error registering strategy {strategy_class.__name__}: {e}")
            return False
    ...
    def validate_strategy_config(self, strategy_id: str, parameters: Dict[str, Any]) -> List[str]:
        ...
        if not strategy:
            ...
            errors.append(f"Unknown strategy: '{strategy_id}'. Available strategies: {', '.join(available_strategies)}")
            return errors
            ...
            if param.name in parameters:
                ...
                if param.param_type is float:
                    if not isinstance(value, (int, float)):
                        errors.append(f"Parameter '{param.name}' must be a number (float or integer), got {type(value).__name__}")
                        continue
                    # Convert int to float for consistency
                    parameters[param.name] = float(value)
                    value = float(value)
                elif not isinstance(value, param.param_type):
                    errors.append(f"Parameter '{param.name}' must be of type {param.param_type.__name__}, got {type(value).__name__}")
                    continue
        ...
        if unexpected_params:
            errors.append(f"Unexpected parameters for strategy '{strategy_id}': {', '.join(unexpected_params)}")
    ...
    def discover_strategies(self) -> int:
        ...
        logger.info(f"Starting strategy discovery across {len(self._strategy_paths)} paths")
        ...
        for strategy_path in self._strategy_paths:
            ...
            for python_file in python_files:
                ...
                try:
                    ...
                    if not spec or not spec.loader:
                        logger.warning(f"Could not create module spec for {python_file}")
                        continue
                    ...
                    for name, obj in inspect.getmembers(module, inspect.isclass):
                        ...
                        if (obj != StrategyInterface and 
                            ...
                            logger.debug(f"Found strategy class: {name} in {python_file.name}")
            ...
            if path_discovered > 0:
                logger.info(f"Discovered {path_discovered} strategies from path: {strategy_path}")
        
        logger.info(f"Strategy discovery completed: {discovered_count} strategies discovered "
    ...
    def _load_core_strategies(self):
        ...
        try:
            ...
            logger.info("Platform will continue without core strategies - they can be loaded later via discovery")
        except Exception as e:
