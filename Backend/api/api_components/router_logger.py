...
class RouterLogger:
    ...
    def log_database_operation(
        self, operation: str, table: str, params: Dict[str, Any] = None
    ):
        ...
        self.logger.debug(
            ...
            extra={"operation": operation, "table": table, "params": params},
        )
    ...
    def log_cache_operation(self, operation: str, cache_key: str, hit: bool = None):
        ...
        if hit is not None:
            extra["cache_hit"] = hit
        self.logger.debug(
            f"Cache operation: {operation} for key {cache_key}", extra=extra
        )

    def log_performance_metric(
        self, endpoint: str, duration_ms: float, operation_type: str = "request"
    ):
        # Log performance metrics for monitoring and optimization
        # Tracks execution time for performance analysis and bottleneck identification
        self.logger.info(
            f"Performance metric: {endpoint} took {duration_ms}ms",
            extra={
                "endpoint": endpoint,
