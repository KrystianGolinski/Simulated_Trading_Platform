...
class SimulationValidator:
    ...
    def __init__(self, stock_repo: StockDataRepository, strategy_service: StrategyServiceInterface):
        ...
        self.stock_repo = stock_repo                                        # Repository for stock data operations and validation
        self.temporal_service = TemporalValidationService(stock_repo)       # Temporal validation for trading periods
        self.error_handler = ErrorHandler()                                 # Structured error handling and categorisation
        self.strategy_service = strategy_service                            # Strategy validation and parameter checking
        
        logger.debug("SimulationValidator initialized with integrated validation services")
    ...
    async def validate_simulation_config(self, config: SimulationConfig) -> ValidationResult:
        ...
        logger.debug(f"Starting validation for simulation config: {len(config.symbols)} symbols, "
        ...
        try:
            ...
            if not errors:
                ...
                temporal_info = await self.temporal_service.get_temporal_info_for_logging(
            ...
            logger.error(f"Validation system error: {error.message} | Context: {error.context}")
    ...
    async def _validate_symbols(self, symbols: List[str]) -> List[ValidationError]:
        # Comprehensive stock symbol validation including existence, format, and duplicates
        # Critical validation that blocks simulation if symbols are invalid
        # Performs database lookup to verify symbol availability
        errors = []
        ...
        if not symbols:
            errors.append(ValidationError(
                field="symbols",
                message="At least one stock symbol is required for simulation",
                error_code="SYMBOLS_EMPTY"
            ))
            return errors
        
        # Symbol count validation (prevent resource exhaustion)
        if len(symbols) > 50:  # Based on model constraints
            errors.append(ValidationError(
                field="symbols",
                message=f"Too many symbols ({len(symbols)}). Maximum allowed is 50",
                error_code="SYMBOLS_LIMIT_EXCEEDED"
            ))
        
        # Check for duplicate symbols (case-insensitive)
        unique_symbols = set(s.upper().strip() for s in symbols if s.strip())
        ...
        if len(unique_symbols) != len(clean_symbols):
            ...
            errors.append(ValidationError(
                field="symbols",
                message=f"Duplicate symbols are not allowed: {', '.join(set(duplicates))}",
        ...
        for symbol in symbols:
            if not symbol or not symbol.strip():
                errors.append(ValidationError(
                    field="symbols",
                    message="Empty or whitespace-only symbols are not allowed",
                    error_code="SYMBOL_INVALID_FORMAT"
                ))
                continue
            
            # Check symbol format (alphanumeric, reasonable length)
            clean_symbol = symbol.strip().upper()
            if not clean_symbol.isalnum() or len(clean_symbol) > 10:
                errors.append(ValidationError(
                    field="symbols",
                    message=f"Invalid symbol format: '{symbol}'. Symbols must be alphanumeric and <= 10 characters",
                    error_code="SYMBOL_INVALID_FORMAT"
                ))
        
        # Database existence validation (only if basic validation passes)
        if not errors:
            try:
                ...
                logger.error(f"Symbol validation database error: {error.message} | Context: {error.context}")
                errors.append(ValidationError(
                    field="symbols",
    ...
    def run_comprehensive_validation_tests(self) -> Dict[str, Any]:
        ...
        for category_name, test_function in test_categories:
            ...
            try:
                ...
                print(f"  {status_symbol} {category_name}: {category_results['passed']}/{category_results['total']} passed ({success_rate:.1f}%)")
                
                # Log detailed failures for debugging
                if category_results['failed'] > 0 and 'details' in category_results:
                    failed_details = [detail for detail in category_results['details'] if 'FAIL' in detail or 'ERROR' in detail]
        ...
        if test_results['failed_tests'] == 0 and test_results['total_tests'] > 0:
            ...
            print(f"🎉 ALL VALIDATION TESTS PASSED ({test_results['passed_tests']}/{test_results['total_tests']}) - {overall_success_rate:.1f}% success rate")
        else:
            test_results['overall_status'] = 'FAILED'
            print(f"❌ VALIDATION TESTS FAILED ({test_results['failed_tests']}/{test_results['total_tests']} failures) - {overall_success_rate:.1f}% success rate")
    ...
    def _test_edge_cases(self) -> Dict[str, int]:
        ...
        for capital, expected_valid, description in boundary_tests:
            ...
            try:
                ...
                if actual_valid == expected_valid:
                    results['passed'] += 1
                    results['details'].append(f"PASS: {description} (capital: {capital})")
                else:
                    results['failed'] += 1
                    error_messages = [e.message for e in validation_errors] if validation_errors else []
        ...
        for symbols, description, max_time in performance_tests:
            ...
            try:
                ...
                if execution_time <= max_time:
                    results['passed'] += 1
                    results['details'].append(f"PASS: {description} ({execution_time:.3f}s <= {max_time}s)")
                else:
                    results['failed'] += 1
                    results['details'].append(f"FAIL: {description} - Too slow: {execution_time:.3f}s > {max_time}s")
    ...
    async def check_database_connection(self) -> ValidationResult:
        ...
        try:
            ...
            logger.debug(f"Database connectivity confirmed: {total_count} total symbols available")
            
            # Assess data adequacy for validation operations
            if total_count < 10:
                warnings.append(f"Very limited symbol data available ({total_count} symbols). "
            ...
            if total_count < 1000:
                warnings.append(f"Relatively small historical dataset detected ({total_count} symbols). "
            ...
            logger.info(f"Database health check completed successfully: {total_count} symbols, {len(warnings)} warnings")
            ...
            logger.error(f"Database connection health check failed: {error.message} | Context: {error.context}")
            errors.append(ValidationError(
                field="database",
