...
class DatabaseManager:
    ...
    def __init__(self):
        ...
        self.stock_data_repository = StockDataRepository(
            self.query_executor, self.cache_manager
        )  # Stock data access
    ...
    async def log_trade(
        ...
        trade_id = await self.query_executor.execute_fetchval(
            query, session_id, symbol, timestamp, action, quantity, price, commission
        )
        return trade_id

    async def get_session_trades(
        self, session_id: int, page: int = 1, page_size: int = 100
    ) -> Dict[str, Any]:
        ...
        trades = await self.query_executor.execute_query(
            query, session_id, page_size, offset
        )
    ...
    async def get_session_results(self, session_id: int) -> Optional[Dict[str, Any]]:
        ...
        trades_result = await self.get_session_trades(
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "connection_pool_size": self.connection_manager.get_pool_stats().get(
                    "connection_count", 0
                ),
