...
class RouterServiceFactory:
    ...
    def get_router_logger(self, router_name: str) -> RouterLogger:
        ...
        if router_name not in self._router_loggers:
            self._router_loggers[router_name] = EndpointLogger.create_router_logger(
                router_name
            )
            logger.debug(f"Created RouterLogger for {router_name}")
        return self._router_loggers[router_name]

    def create_router_base(
        self, router_name: str, custom_services: Optional[Dict[str, Any]] = None
    ) -> RouterBase:
        """
        ...
        validation_service = (
            services.get("validation_service") or self.get_validation_service()
        )
        response_formatter = (
            services.get("response_formatter") or self.get_response_formatter()
        )
        router_logger = services.get("router_logger") or self.get_router_logger(
            router_name
        )
