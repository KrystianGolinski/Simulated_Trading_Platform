...
class BollingerBandsStrategy(StrategyInterface):
    ...
    def validate_parameters(self, parameters: Dict[str, Any]) -> List[str]:
        ...
        if bb_period and bb_period < 15:
            errors.append("Bollinger Bands period should be at least 15 for stable calculations")
        
        # Validate deviation range
        if bb_deviation and (bb_deviation < 1.5 or bb_deviation > 2.5):
            errors.append("Bollinger Bands deviation should be between 1.5 and 2.5 for meaningful signals")
        
        # Validate oversold percentage
        if bb_oversold_pct and bb_oversold_pct > 10.0:
            errors.append("Oversold percentage should not exceed 10% for practical trading")
