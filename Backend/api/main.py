...
from routers import engine, health, performance, simulation, stocks, strategies
...
class ValidationError(Exception):
    ...
    def __init__(self, message: str, errors: list = None):
        ...
        self.errors = (
            errors or []
        )  # List of ApiError objects for detailed validation feedback
...
async def operation_exception_handler(request: Request, exc: OperationError):
    ...
    response = _global_response_formatter.create_error_response(
        "Operation failed", [ApiError(code=exc.code, message=exc.message)]
    )
...
async def value_error_handler(request: Request, exc: ValueError):
    ...
    response = _global_response_formatter.create_error_response(
        "Invalid input provided", [ApiError(code="INVALID_INPUT", message=str(exc))]
    )
    return JSONResponse(status_code=400, content=response.dict())
...
app.include_router(
    simulation.router
)  # Simulation lifecycle and parallel execution (/simulation/*)
app.include_router(
    performance.router
)  # Performance analytics and optimization metrics (/performance/*)
app.include_router(engine.router)  # C++ engine interface and testing (/engine/*)
app.include_router(
    strategies.router
)  # Strategy management and discovery (/strategies/*)
