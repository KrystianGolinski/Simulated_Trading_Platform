...
class StrategyFactory:
    ...
    def __init__(self):
        ...
        if discovered > 0:
            logger.info(f"Discovered {discovered} additional strategies during factory initialization")
        
        # Log total available strategies for monitoring
        total_strategies = len(self.registry.get_available_strategies())
        logger.info(f"Strategy factory initialized with {total_strategies} total strategies available")
    ...
    def get_available_strategies(self) -> StandardResponse[Dict[str, Any]]:
        ...
        try:
            ...
            return create_success_response(
                {
                    ...
                    "categories": list(set(s["category"] for s in strategy_list)),  # Available categories
            ...
            return create_error_response(
                ...
                [ApiError(code="STRATEGY_RETRIEVAL_ERROR", message=str(e), details={"operation": "get_available_strategies"})]
            )
    ...
    def get_strategy_metadata(self, strategy_id: str) -> StandardResponse[Dict[str, Any]]:
        ...
        try:
            ...
            if strategy_id not in strategies:
                ...
                logger.warning(f"Strategy '{strategy_id}' not found. Available strategies: {available_strategies}")
                ...
                "parameters": [
                    {
                        ...
                        "validation_rules": {                           # Additional validation info
                            ...
                            "range_validation": param.min_value is not None or param.max_value is not None
                        }
                    }
                    for param in metadata.parameters
                ],
            ...
            return create_error_response(
                ...
                [ApiError(
                    ...
                    message=str(e),
                    details={"strategy_id": strategy_id, "operation": "get_strategy_metadata"}
                )]
            )
    ...
    def validate_strategy_config(self, strategy_id: str, parameters: Dict[str, Any]) -> StandardResponse[Dict[str, Any]]:
        ...
        try:
            ...
            if strategy_id not in available_strategies:
                ...
                logger.warning(f"Validation requested for unknown strategy '{strategy_id}'. Available: {available_ids}")
            ...
            if validation_errors:
                logger.info(f"Strategy configuration validation failed for '{strategy_id}': {len(validation_errors)} errors")
                
                # Create detailed error responses for each validation failure
                error_details = []
                for error in validation_errors:
                    error_details.append(ApiError(
                        code="VALIDATION_FAILED", 
                        message=error,
                        details={"strategy_id": strategy_id, "parameter_context": "validation"}
                    ))
            ...
            if not strategy:
                logger.error(f"Failed to instantiate strategy '{strategy_id}' after successful validation")
                return create_error_response(
                    ...
                    [ApiError(
                        ...
                        details={"strategy_id": strategy_id, "validation_stage": "post_validation"}
                    )]
                )
            ...
            try:
                ...
                logger.debug(f"Successfully transformed parameters for strategy '{strategy_id}'")
            except Exception as transform_error:
                logger.error(f"Parameter transformation failed for '{strategy_id}': {transform_error}")
                return create_error_response(
                    f"Parameter transformation failed for strategy '{strategy_id}'",
                    [ApiError(
                        code="PARAMETER_TRANSFORMATION_ERROR",
                        message=str(transform_error),
                        details={"strategy_id": strategy_id, "original_parameters": parameters}
                    )]
                )
            ...
            logger.error(f"Unexpected error during strategy validation for '{strategy_id}': {e}")
            return create_error_response(
    ...
    def create_strategy_config(self, strategy_id: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            strategy = self.registry.get_strategy(strategy_id)
            if not strategy:
                logger.error(f"Strategy '{strategy_id}' not found during configuration creation")
                raise ValueError(f"Strategy '{strategy_id}' not found in registry")
            
            # Transform parameters to C++ engine format
            logger.debug(f"Creating C++ compatible configuration for strategy '{strategy_id}'")
            transformed_config = strategy.transform_parameters(parameters)
            
            # Validate transformation result
            if not isinstance(transformed_config, dict):
                raise ValueError(f"Strategy '{strategy_id}' transformation returned invalid format: {type(transformed_config)}")
            
            logger.debug(f"Successfully created strategy configuration for '{strategy_id}': {list(transformed_config.keys())}")
            return transformed_config
            
        except Exception as e:
            logger.error(f"Failed to create strategy config for '{strategy_id}': {e}")
            raise ValueError(f"Strategy configuration creation failed for '{strategy_id}': {str(e)}")
    ...
    def refresh_strategies(self) -> StandardResponse[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Starting strategy registry refresh from {initial_count} strategies")
            ...
            new_strategy_ids = [sid for sid in final_strategies.keys() if sid not in initial_strategies]
            
            logger.info(f"Strategy registry refresh completed: {discovered_count} scanned, "
            ...
            return create_success_response(
                {
                    ...
                    "scan_paths": len(self.registry._strategy_paths)  # Number of paths scanned
                },
