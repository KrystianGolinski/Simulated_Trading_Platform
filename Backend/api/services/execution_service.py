...
class ExecutionService:
    ...
    def create_config_file(self, config: SimulationConfig) -> str:
        ...
        try:
            ...
            logger.debug(
                f"Created config file: {config_file} with strategy: {config.strategy}"
            )
            return config_file
    ...
    def build_cpp_command(self, config: SimulationConfig) -> tuple[List[str], str]:
        ...
        cmd = [str(self.cpp_engine_path), "--simulate", "--config", config_file]
    ...
    async def execute_simulation(
        ...
        try:
            ...
            if not working_dir.exists() or not working_dir.is_dir():
                raise RuntimeError(
                    f"Invalid working directory for C++ engine: {working_dir}"
                )

            logger.debug(
                f"Starting simulation {simulation_id}: {' '.join(cmd)} (build_time: {build_time:.2f}ms)"
            )
            ...
            stdout_data, stderr_data = await self._read_process_streams(
                process, simulation_id
            )
        ...
        async def read_stderr():
            while True:
                ...
                if simulation_id in self.active_simulations:
                    ...
                    sim_info["last_heartbeat"] = (
                        datetime.now()
                    )  # Any output counts as heartbeat
                ...
                try:
                    ...
                    if progress_data.get("type") == "progress":
                        if simulation_id in self.active_simulations:
                            sim_info = self.active_simulations[simulation_id]
                            sim_info["progress_pct"] = progress_data.get(
                                "progress_pct", 0
                            )
                            sim_info["current_date"] = progress_data.get("current_date")
                            sim_info["current_value"] = progress_data.get(
                                "current_value"
                            )

                            logger.info(
                                f"Simulation {simulation_id} progress: {progress_data.get('progress_pct', 0):.1f}%"
                            )
        ...
        try:
            ...
            for line in lines:
                ...
                # Parse portfolio memory
                    ...
                    "Portfolio Memory Usage:" in line
                    and "Estimated memory:" in memory_report
                ):
                    # Look for the estimated memory line after portfolio section
                    portfolio_section = memory_report[
                        memory_report.find("Portfolio Memory Usage:") :
                    ]
                    for portfolio_line in portfolio_section.split("\n"):
                        if "Estimated memory:" in portfolio_line:
                            try:
                                memory_stats["portfolio_memory_bytes"] = int(
                                    portfolio_line.split()[-2]
                                )
                            except (ValueError, IndexError):
                                pass
                            break
                ...
                # Parse service-specific memory usage
                    ...
                    market_data_section = memory_report[
                        memory_report.find("MarketData Memory Usage:") :
                    ]
                    for md_line in market_data_section.split("\n"):
                        if "Estimated memory:" in md_line:
                            try:
                                memory_stats["market_data_cache_bytes"] = int(
                                    md_line.split()[-2]
                                )
                            except (ValueError, IndexError):
                                pass
                    ...
                    exec_section = memory_report[
                        memory_report.find("ExecutionService Memory Usage:") :
                    ]
                    for exec_line in exec_section.split("\n"):
                        if "Total estimated memory:" in exec_line:
                            try:
                                memory_stats["execution_service_bytes"] = int(
                                    exec_line.split()[-2]
                                )
                            except (ValueError, IndexError):
                                pass
                    ...
                    pa_section = memory_report[
                        memory_report.find("PortfolioAllocator Memory Usage:") :
                    ]
                    for pa_line in pa_section.split("\n"):
                        if "Estimated memory:" in pa_line:
                            try:
                                memory_stats["portfolio_allocator_bytes"] = int(
                                    pa_line.split()[-2]
                                )
                            except (ValueError, IndexError):
                                pass
                    ...
                    dp_section = memory_report[
                        memory_report.find("DataProcessor Memory Usage:") :
                    ]
                    for dp_line in dp_section.split("\n"):
                        if "Total estimated memory:" in dp_line:
                            try:
                                memory_stats["data_processor_bytes"] = int(
                                    dp_line.split()[-2]
                                )
                            except (ValueError, IndexError):
                                pass
    ...
    async def cancel_simulation(self, simulation_id: str) -> bool:
        ...
        if simulation_id not in self.active_simulations:
            logger.warning(
                f"Cannot cancel simulation {simulation_id}: not found in active simulations"
            )
            return False
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Simulation {simulation_id} didn't terminate gracefully, force killing"
                )
                process.kill()
                await process.wait()
