...
class TemporalValidationService:
    ...
    async def is_stock_tradeable(self, symbol: str, check_date: date) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error checking if stock {symbol} was tradeable on {check_date}: {e}"
            )
            return False

    async def validate_period_eligibility(
        ...
        """
        try:
            return await self.stock_repo.validate_symbols_for_period(
            ...
            logger.error(
                f"Temporal validation error: {error.message} | Context: {error.context}"
            )
    ...
    async def get_temporal_info_for_logging(
        self, symbols: List[str], start_date: date, end_date: date
    ) -> Dict[str, List[str]]:
        """
        ...
        try:
            ...
            for symbol in symbols_to_check:
                try:
                    temporal_info = await self.stock_repo.get_stock_temporal_info(
                        symbol
                    )
                    if temporal_info:
                        ...
                        if ipo_date_str:
                            try:
                                ipo_date_obj = datetime.fromisoformat(
                                    ipo_date_str
                                ).date()
                                ...
                                if ipo_date_obj > start_date:
                                    stocks_with_delayed_ipo.append(
                                        {"symbol": symbol, "ipo_date": ipo_date_str}
                                    )
                        ...
                        if delisting_date_str:
                            try:
                                delisting_date_obj = datetime.fromisoformat(
                                    delisting_date_str
                                ).date()
            ...
            if stocks_with_early_delisting:
                delisted_symbols = [
                    s["symbol"] for s in stocks_with_early_delisting[:3]
                ]
                warnings.append(
            ...
            if (
                not stocks_with_delayed_ipo
                and not stocks_with_early_delisting
                and symbols_to_check
            ):
                warnings.append(
    ...
    async def validate_temporal_eligibility(
        ...
        try:
            ...
            start_tradeable = await self.stock_repo.validate_stock_tradeable(
                symbol, start_date
            )
            ...
            end_tradeable = await self.stock_repo.validate_stock_tradeable(
                symbol, end_date
            )
    ...
    async def check_ipo_proximity_warning(
        self, symbol: str, start_date: date, days_threshold: int = 90
    ) -> Optional[str]:
        """
    ...
    async def get_eligible_stocks_for_period(
        ...
        """
        try:
            return await self.stock_repo.get_eligible_stocks_for_period(
                start_date, end_date
            )
        except Exception as e:
            logger.error(
                f"Error getting eligible stocks for period {start_date} to {end_date}: {e}"
            )
            return []

    async def batch_temporal_validation(
        self, symbols: List[str], start_date: date, end_date: date
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            validation_result = await self.stock_repo.validate_symbols_for_period(
                symbols, start_date, end_date
            )

            # Add business logic analysis
            rejection_rate = validation_result["total_rejected"] / max(
                validation_result["total_requested"], 1
            )
            ...
            logger.error(
                f"Batch temporal validation error: {error.message} | Context: {error.context}"
            )
            ...
            return {
                ...
                "business_warnings": [
                    "Temporal validation system encountered an error"
                ],
                "period_analysis": {
                    "start_date": start_date.isoformat(),
                    "end_date": end_date.isoformat(),
    ...
    async def validate_simulation_temporal(
        self, symbols: List[str], start_date: date, end_date: date
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if rejection_rate > 0.5:
                logger.warning(
                    f"High symbol rejection rate: {rejection_rate:.1%} for period {start_date} to {end_date}"
                )
            ...
            logger.error(
                f"Simulation temporal validation error: {error.message} | Context: {error.context}"
            )
