...
class PerformanceCalculator:
    ...
    def calculate_performance_metrics(
        ...
        return PerformanceMetrics(
            ...
            max_drawdown_pct=performance_data.get(
                "max_drawdown_pct", performance_data.get("max_drawdown", 0.0)
            ),
            win_rate=performance_data.get("win_rate", 0.0),
            total_trades=performance_data.get(
                "total_trades", performance_data.get("trades", 0)
            ),
            ...
            max_drawdown=performance_data.get(
                "max_drawdown"
            ),  # Absolute drawdown value
    ...
    def validate_performance_metrics(self, performance_metrics: Any) -> bool:
        ...
        metric_validations = [
            ...
            (
                ...
                float,
                lambda x: x >= 0,
            ),  # Win rate can be percentage or ratio
        ...
        for field, field_type, validator in metric_validations:
            if field in performance_metrics:
                ...
                if field_type == float and not isinstance(value, (int, float)):
                    logger.error(
                        f"Performance metric '{field}' must be numeric, got {type(value)}"
                    )
                    return False
                elif field_type == int and not isinstance(value, int):
                    logger.error(
                        f"Performance metric '{field}' must be integer, got {type(value)}"
                    )
                    return False
                ...
                if validator and not validator(value):
                    logger.error(
                        f"Performance metric '{field}' failed validation: {value}"
                    )
                    return False

        # Cross-field validation for trade count consistency (flexible for different methodologies)
        if (
            "winning_trades" in performance_metrics
            and "losing_trades" in performance_metrics
            and "total_trades" in performance_metrics
        ):
            ...
            if winning + losing > total:
                logger.error(
                    f"Trade count impossible: winning({winning}) + losing({losing}) > total({total})"
                )
                return False
            elif winning + losing < total:
                # Acceptable scenario - may include neutral trades or open positions
                logger.info(
                    f"Trade count info: winning({winning}) + losing({losing}) < total({total}) - may include neutral/open trades"
                )
    ...
    def validate_cross_field_consistency(self, result_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if starting_capital and ending_value and total_return_pct is not None:
                ...
                expected_return = (
                    (ending_value - starting_capital) / starting_capital
                ) * 100
                if (
                    abs(expected_return - total_return_pct) > 0.01
                ):  # Allow small floating point differences
                    logger.warning(
                        f"Return percentage inconsistency: expected {expected_return:.2f}%, got {total_return_pct:.2f}%"
                    )
                ...
                if equity_curve and len(equity_curve) > 0:
                    ...
                    if final_equity_value and ending_value:
                        if abs(final_equity_value - ending_value) > 0.01:
                            logger.warning(
                                f"Equity curve final value ({final_equity_value}) doesn't match ending_value ({ending_value})"
                            )
