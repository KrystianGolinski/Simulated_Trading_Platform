...
class ErrorCategorizer(ABC):
    ...
    @abstractmethod
    def categorize(
        self, return_code: int, stdout: str, stderr: str, context: Dict[str, Any]
    ) -> SimulationError:
        """
...
class CppErrorExtractor:
    ...
    def extract_cpp_error_details(self, stderr: str, stdout: str) -> Dict[str, Any]:
        ...
        cpp_error_patterns = [
            ...
            (
                r"terminate called after throwing an instance of '([^']+)'\s*what\(\):\s*(.+)",
                "exception_type",
                "error_message",
            ),
            (r"Exception:\s*([^\n]+)", None, "error_message"),
        ...
        if any(
            db_keyword in combined_output.lower()
            for db_keyword in ["sqlite", "database", "sql"]
        ):
