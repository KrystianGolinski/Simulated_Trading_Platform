...
class ResultProcessor:
    ...
    def initialize_simulation_result(
    ...
    def update_simulation_status(
        ...
        if simulation_id not in self.results_storage:
            logger.warning(
                f"Attempting to update status for unknown simulation: {simulation_id}"
            )
            return
    ...
    def process_simulation_results(
        ...
        if simulation_id not in self.results_storage:
            logger.error(
                f"Cannot process results for unknown simulation: {simulation_id}"
            )
            return
        ...
        try:
            ...
            simulation_result.performance_metrics = (
                self.performance_calculator.calculate_performance_metrics(result_data)
            )

            # Process trading signals into structured trade records using specialized converter
            signals_data = result_data.get("signals", [])
            simulation_result.trades = self.trade_converter.convert_signals_to_trades(
                signals_data, result_data
            )

            # Process equity curve data using specialized processor
            simulation_result.equity_curve = self.equity_processor.process_equity_curve(
                result_data
            )
            ...
            logger.info(
                f"Successfully processed results for simulation {simulation_id}"
            )

        except Exception as e:
            logger.error(
                f"Error processing simulation results for {simulation_id}: {e}"
            )
            self.mark_simulation_failed(
                simulation_id, f"Result processing error: {str(e)}"
            )
    ...
    def mark_simulation_failed(self, simulation_id: str, error_message: str):
        ...
        if simulation_id not in self.results_storage:
            logger.warning(
                f"Attempting to mark unknown simulation as failed: {simulation_id}"
            )
            return
    ...
    def validate_result_data(self, result_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if "performance_metrics" in result_data:
                if not self.performance_calculator.validate_performance_metrics(
            ...
            if "equity_curve" in result_data:
                if not self.equity_processor.validate_equity_curve(
                    result_data["equity_curve"]
                ):
                    return False

            # Cross-field consistency validation using specialized calculator
            if not self.performance_calculator.validate_cross_field_consistency(
                result_data
            ):
                return False
    ...
    def _pre_validate_json_text(self, json_text: str):
        ...
        if not json_text.strip().startswith("{"):
            if any(
                error_indicator in json_text.lower()
        ...
        if bracket_count != 0:
            raise ValueError(
                f"Unbalanced brackets in JSON (difference: {bracket_count})"
            )
