        ...
        response = router_base.response_formatter.create_error_response(
            ...
            [
                ApiError(
                    ...
                    message=f"Engine binary not found at {engine_path}",
                    field="engine_path",
                )
            ],
        )
        router_base.router_logger.log_error(
            "/engine/test", Exception("Engine not found"), "ENGINE_NOT_FOUND"
        )
        return response
    ...
    try:
        ...
        if result.returncode == 0:
            return router_base.success_response(
                "/engine/test", test_data, "Engine test completed successfully"
            )
        else:
            response = router_base.response_formatter.create_error_response(
                "Engine test failed",
                [
                    ApiError(
                        ...
                        message=f"Engine returned code {result.returncode}",
                    )
                ],
            )
            router_base.router_logger.log_error(
                "/engine/test",
                Exception(f"Engine returned code {result.returncode}"),
                "ENGINE_TEST_FAILED",
            )
            return response
        ...
        return router_base.response_formatter.create_error_response(
            "Engine test error", [ApiError(code="ENGINE_TEST_ERROR", message=str(e))]
        )
...
async def get_engine_status() -> StandardResponse[Dict[str, Any]]:
    ...
    try:
        ...
        engine_path_info = {
            "cpp_engine_path": (
                str(simulation_engine.cpp_engine_path)
                if simulation_engine.cpp_engine_path
                else None
            ),
            "path_exists": (
                simulation_engine.cpp_engine_path.exists()
                if simulation_engine.cpp_engine_path
                else False
            ),
            "path_executable": (
                os.access(simulation_engine.cpp_engine_path, os.X_OK)
                if simulation_engine.cpp_engine_path
                and simulation_engine.cpp_engine_path.exists()
                else False
            ),
        ...
        for dir_path in possible_dirs:
            if dir_path.exists():
                try:
                    ...
                    directory_contents[str(dir_path)] = contents[
                        :10
                    ]  # Limit to first 10 files for response size
                except Exception as e:
        ...
        router_base.log_request("/engine/status")

        # Format response based on engine validation status
        if engine_path_info["is_valid"]:
            return router_base.success_response(
                "/engine/status", status_data, "Engine status retrieved successfully"
            )
        else:
            response = router_base.response_formatter.create_error_response(
                ...
                [
                    ApiError(
                        code="ENGINE_INVALID",
                        message="Engine is not valid or not accessible",
                    )
                ],
            )
            router_base.router_logger.log_error(
                "/engine/status", Exception("Engine invalid"), "ENGINE_INVALID"
            )
            return response
    except Exception as e:
